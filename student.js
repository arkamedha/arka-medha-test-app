    ...
    try {
        ...
        querySnapshot.forEach((doc) => {
            ...
            if(qType === "MCQ") {
                ...
                inputHTML = `<textarea id="ans_${qId}" class="subjective-textarea" placeholder="Type answer here or write 'Done in Notebook'..."></textarea>`;
            }
...
submitTestBtn.addEventListener("click", async () => {
    ...
    submitTestBtn.innerText = "Submitting... Please Wait ⏳"; 
    ...
    for (let qId in correctAnswers) {
        ...
        detailedResponses.push({ 
            qNum: qIndex, question: qData.question, selected: selectedOption, 
            correct: qData.answer, marks: qData.marks, qType: qData.type, 
            isCorrect: isCorrect
        });
        qIndex++;
    }
    ...
    try {
        ...
        if(hasSubjective) {
            document.getElementById("resultMessage").innerHTML = `Test Submitted Successfully! 🎉<br><span style="color:orange;">Subjective Answers Pending Teacher Review (Check Notebook).</span>`;
        } else {
...
document.getElementById("viewMyResultsBtn").addEventListener("click", async () => {
    ...
    try {
        ...
        snaps.forEach(doc => {
            ...
            if(status === "PENDING") {
                html += `<div class="res-card" style="background: #e9ecef; padding: 10px; margin-top: 10px; border-left: 4px solid orange; border-radius: 4px;">
                            <strong>Test: ${data.testName}</strong><br>Date: ${data.date}<br>
                            <span style="color:orange; font-weight:bold;">⏳ Result Pending Teacher Review</span>
                         </div>`;
            } else {
                html += `<div class="res-card" style="background: #e9ecef; padding: 10px; margin-top: 10px; border-left: 4px solid green; border-radius: 4px;">
                            <strong>Test: ${data.testName}</strong><br>Date: ${data.date}<br>
